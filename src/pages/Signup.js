...
import { Eye, EyeOff, User, Building2, Settings, Check } from 'lucide-react';
...
const Signup = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        const dashboardPath = formData.role === 'candidate' ? '/candidate/dashboard' :
                             formData.role === 'company' ? '/company/dashboard' : '/admin/dashboard';
  ...
  const roleOptions = [
    { value: 'candidate', label: 'Candidate', icon: <User className="w-5 h-5" /> },
    { value: 'company', label: 'Company', icon: <Building2 className="w-5 h-5" /> },
    { value: 'admin', label: 'Administrator', icon: <Settings className="w-5 h-5" /> }
  ];
                ...
                {roleOptions.map((option) => (
                  ...
                  >
                    <input
                      ...
                      className="sr-only"
                    />
                    <div className={`mb-2 ${formData.role === option.value ? 'text-primary' : 'text-gray-400'}`}>
                      {option.icon}
                    </div>
                    <span className={`text-sm font-medium ${
                      formData.role === option.value ? 'text-primary' : 'text-gray-700'
                    }`}>
                      {option.label}
                    </span>
                  </label>
