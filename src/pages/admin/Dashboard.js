...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    setTopCompanies([
      { name: 'TechCorp India', applications: 245, shortlisted: 45, selected: 12 },
      { name: 'DataViz Solutions', applications: 189, shortlisted: 38, selected: 8 },
  ...
  const getActivityIcon = (type) => {
    switch (type) {
      case 'matching': return <Target className="w-5 h-5 text-blue-500" />;
      case 'quota': return <PieChart className="w-5 h-5 text-yellow-500" />;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 mt-2">
            Monitor system performance, manage quotas, and oversee the allocation process.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
          <div className="card text-center">
            <div className="text-3xl font-bold text-primary mb-2">{stats.totalCandidates}</div>
            <div className="text-gray-600">Total Candidates</div>
          </div>
          <div className="card text-center">
            <div className="text-3xl font-bold text-accent mb-2">{stats.totalCompanies}</div>
            <div className="text-gray-600">Partner Companies</div>
          </div>
          <div className="card text-center">
            <div className="text-3xl font-bold text-yellow-600 mb-2">{stats.totalPostings}</div>
            <div className="text-gray-600">Active Postings</div>
          </div>
          <div className="card text-center">
            <div className="text-3xl font-bold text-green-600 mb-2">{stats.quotaFilled}%</div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="card">
              ...
              <div className="grid md:grid-cols-2 gap-4">
                <Link to="/admin/quota-config" className="card hover:shadow-lg transition-shadow duration-300 group">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">Manage quota settings and policies</p>
                    </div>
                  </div>
                </Link>

                <Link to="/admin/simulator" className="card hover:shadow-lg transition-shadow duration-300 group">
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-accent/10 rounded-lg flex items-center justify-center group-hover:bg-accent/20 transition-colors">
                      <BarChart3 className="w-6 h-6 text-accent" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-gray-900">What-If Simulator</h3>
                      <p className="text-sm text-gray-600">Test different quota scenarios</p>
                    </div>
                  </div>
                </Link>

                <Link to="/admin/batch-matching" className="card hover:shadow-lg transition-shadow duration-300 group">
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center group-hover:bg-purple-200 transition-colors">
                      <Target className="w-6 h-6 text-purple-600" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-gray-900">Batch Matching</h3>
                      <p className="text-sm text-gray-600">Run automated matching process</p>
                    </div>
                  </div>
                </Link>

                <Link to="/admin/allocation-review" className="card hover:shadow-lg transition-shadow duration-300 group">
                  <div className="flex items-center space-x-4">
                    <div className="w-12 h-12 bg-orange-100 rounded-lg flex items-center justify-center group-hover:bg-orange-200 transition-colors">
                      <CheckCircle className="w-6 h-6 text-orange-600" />
                    </div>
                    <div>
                      <h3 className="font-semibold text-gray-900">Allocation Review</h3>
                      <p className="text-sm text-gray-600">Review and approve allocations</p>
                    </div>
                  </div>
                </Link>
              </div>
            ...
            {/* Quota Status */}
            <div className="card">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">Quota Status</h2>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        <span className="text-sm font-medium text-gray-900 capitalize">
                          {category === 'obc' ? 'OBC' : category === 'sc' ? 'SC' : category === 'st' ? 'ST' : category === 'ews' ? 'EWS' : category}
                        </span>
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full transition-all duration-300 ${
                            ...
                            data.percentage >= 60 ? 'bg-yellow-500' : 'bg-gray-400'
                          }`}
            ...
            {/* Top Companies */}
            <div className="card">
              <h2 className="text-xl font-semibold text-gray-900 mb-6">Top Performing Companies</h2>
              <div className="space-y-4">
            ...
            <div className="card">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{stats.avgMatchScore}%</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-semibold text-gray-900">{stats.responseTime} days</span>
                </div>
                <div className="flex items-center justify-between">
                ...
                {topSkills.slice(0, 5).map((skill, index) => (
                  <div key={index} className="flex items-center justify-between">
                    ...
                    <div className={`text-sm font-bold ${
                      ...
                      skill.gap > 10 ? 'text-yellow-600' : 'text-green-600'
                    }`}>
                      {skill.gap}%
                    </div>
                  </div>
