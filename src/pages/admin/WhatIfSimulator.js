...
const WhatIfSimulator = () => {
	...
	const getQuotaStatus = (percentage) => {
		if (percentage >= 100)
			return { status: "exceeded", color: "text-red-600 bg-red-100" };
		if (percentage >= 80)
	...
	const quotaCategories = [
		{
			key: "general",
			label: "General",
			description: "Open category for all candidates",
		},
		...
		{ key: "ews", label: "EWS", description: "Economically Weaker Sections" },
		{
			key: "women",
			label: "Women",
			description: "Reservation for women candidates",
		},
	];
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				...
				<div className="mb-8">
					...
					<p className="text-gray-600 mt-2">
						Test different quota scenarios and analyze their impact on the
						allocation process.
					</p>
				</div>
								...
								<div className="space-y-4">
									...
									<div className="grid grid-cols-2 gap-4">
										<div className="text-center p-3 bg-blue-50 rounded-lg">
											<div className="text-lg font-bold text-blue-600">
												{simulationResults.totalAllocated}
											</div>
									...
									<div className="space-y-2">
										...
										<div className="space-y-1 text-sm text-gray-600">
											<div>
												New Allocations:{" "}
												{simulationResults.impactAnalysis.newAllocations}
											</div>
											<div>
												Reallocations:{" "}
												{simulationResults.impactAnalysis.reallocations}
											</div>
											<div>
												Affected Candidates:{" "}
												{simulationResults.impactAnalysis.affectedCandidates}
											</div>
											<div>
												Companies Affected:{" "}
												{simulationResults.impactAnalysis.companiesAffected}
											</div>
										</div>
									</div>
								</div>
