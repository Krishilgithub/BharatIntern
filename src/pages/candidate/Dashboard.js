...
const StatsCard = memo(({ title, value, icon: Icon, change, loading }) => {
	if (loading) {
		return (
...
const CandidateDashboard = () => {
	...
	const loadRecommendations = useCallback(async () => {
		...
		try {
			...
			const apiPromise = apiService.getRecommendations(user?.id || 1, 10);
			...
			if (response.data && Array.isArray(response.data)) {
				...
				const transformedRecommendations = response.data.map((rec) => ({
					...
					matchScore: rec.matchScore || rec.match_score || 75,
					skills: rec.skills || [],
					deadline: rec.deadline,
					stipend: rec.stipend,
					type: rec.type || "Full-time",
					applicants: rec.applicants || 0,
			...
			// Keep existing recommendations or set empty array
	...
	const loadApplications = useCallback(async () => {
		...
		try {
			...
			const apiPromise = apiService.getApplications(user?.id || 1);
			const response = await Promise.race([apiPromise, timeoutPromise]);
			...
			if (response.data && Array.isArray(response.data)) {
				...
				const transformedApplications = response.data.map((app) => ({
					...
					nextStep: app.nextStep || app.next_step || "Under Review",
					interviewDate: app.interviewDate || app.interview_date || null,
			...
			// Keep existing applications or set empty array
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
					...
					>
						<StatsCard
							title="Applications"
							value={memoizedStats.totalApplications || 0}
							icon={Briefcase}
							change="3"
					...
					>
						<StatsCard
							title="Shortlisted"
							value={memoizedStats.shortlisted || 0}
							icon={CheckCircle}
							change="2"
							loading={loadingStates.stats}
						/>
					...
					>
						<StatsCard
							title="Profile Views"
							value={memoizedStats.profileViews || 0}
							icon={Eye}
							change="12"
							loading={loadingStates.stats}
						/>
					...
					>
						<StatsCard
							title="Skill Rating"
							value={`${memoizedStats.skillRating || 0}/5.0`}
							icon={Star}
							change="0.3"
							loading={loadingStates.stats}
						/>
