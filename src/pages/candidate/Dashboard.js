...
const CandidateDashboard = () => {
  ...
  const loadRecommendations = async () => {
    try {
      // Load real recommendations from backend
      const response = await apiService.getRecommendations(user?.id || 1, 10);
  ...
  const loadApplications = async () => {
    try {
      ...
      const response = await apiService.getApplications(user?.id || 1);
