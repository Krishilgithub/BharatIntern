...
const EnhancedRecommendations = () => {
  ...
  const candidateId =
    window.localStorage.getItem("candidateId") || "demo-candidate"; // Replace with real auth/session
  ...
  const handleAnalyzeResume = async () => {
    ...
    try {
      // Create candidate profile for AI matching
      ...
      const jobsResponse = await apiService.getPostings();
      const jobPostings = jobsResponse.data || [];

      // Use AI matching engine
      const matchResponse = await apiService.matchJobsAI(
        [candidateProfile],
        jobPostings
      );

      if (matchResponse.data.success) {
        // Transform AI matching results to recommendations
        const aiMatches = matchResponse.data.matches[0]?.job_matches || [];
        const transformedRecommendations = aiMatches.map((match) => ({
          id: match.job_id,
          title: match.job_title,
          company: match.company,
          location: match.location,
          matchScore: Math.round(match.match_percentage),
          skills: [], // Will be populated from job posting
          description: match.recommendation_reason,
          matchBreakdown: match.match_breakdown,
          semanticSimilarity: match.semantic_similarity,
          weightedScore: match.weighted_score,
        }));

        setRecommendations(transformedRecommendations);
        setFilteredRecommendations(transformedRecommendations);
        ...
        setAiInsights({
          ...
          averageScore:
            aiMatches.reduce((sum, match) => sum + match.weighted_score, 0) /
            aiMatches.length,
          topSkills: extractTopSkills(aiMatches),
          recommendations: generateAIRecommendations(aiMatches),
        });
      } else {
      ...
      setAnalyzeStatus("Analysis failed. Using fallback data.");
      ...
      const fallbackRecommendations = [
        {
          ...
          description: "Excellent match with your skills",
        },
      ];
  ...
  const extractTopSkills = (matches) => {
    const skillCounts = {};
    matches.forEach((match) => {
      if (match.match_breakdown?.strengths) {
        match.match_breakdown.strengths.forEach((strength) => {
          const skills = strength.match(/skills: ([^,]+)/i);
          if (skills) {
  ...
  const generateAIRecommendations = (matches) => {
    const recommendations = [];

    if (matches.length > 0) {
      const avgScore =
        matches.reduce((sum, match) => sum + match.weighted_score, 0) /
        matches.length;

      if (avgScore > 0.8) {
        recommendations.push(
          "Your profile matches well with available positions"
        );
      } else if (avgScore > 0.6) {
        recommendations.push(
          "Consider improving specific skills to increase match scores"
        );
      } else {
        recommendations.push(
          "Focus on building core skills for better opportunities"
        );
      }
    }
  ...
  useEffect(() => {
    async function fetchRecommendations() {
      try {
        const res = await apiService.getRecommendations(candidateId);
    ...
    setTimeout(() => {
      setAiInsights({
        ...
        perfectMatches: recommendations.filter((r) => r.matchScore >= 90)
          .length,
        averageMatchScore: Math.round(
          recommendations.reduce((acc, r) => acc + r.matchScore, 0) /
    ...
    setAlerts([
      {
        id: 1,
        type: "new_match",
        message: "3 new high-match positions available",
        time: "2 hours ago",
      },
      {
        id: 2,
        type: "deadline",
        message: "Application deadline approaching for 2 positions",
        time: "1 day ago",
      },
    ]);
  }, [candidateId]);
      ...
      if (value) {
        switch (key) {
          ...
          case "salaryRange":
            ...
            filtered = filtered.filter((rec) =>
              rec.salary.includes(value.split("-")[0])
            );
            break;
          case "companySize":
  ...
  const companySizes = [
    ...new Set(recommendations.map((rec) => rec.companySize)),
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-gray-600 mt-2">
                Discover personalized opportunities tailored to your profile and
                career goals
              </p>
            </div>
        ...
        >
          <div className="flex items-center space-x-4">
            <p className="text-gray-600">
              ...
              of <span className="font-semibold">{recommendations.length}</span>{" "}
              recommendations
            </p>
        ...
        <div
          className={
            viewMode === "grid" ? "grid lg:grid-cols-2 gap-6" : "space-y-4"
          }
        >
          <AnimatePresence>
                      ...
                      <span
                        ...
                        className={`px-2 py-1 text-xs rounded-full ${
                          personalPreferences.careerGoals.some((goal) =>
                            skill
                              .toLowerCase()
                              .includes(goal.toLowerCase().split(" ")[0])
                          )
                    ...
                    <div className="text-center">
                      <div
                        className={`text-2xl font-bold ${
                          getMatchScoreColor(rec.matchScore).split(" ")[0]
                        }`}
                      >
                        {rec.matchScore}%
                      </div>
              ...
              >
                ...
                <div className="p-6">
                  ...
                  <div className="text-gray-600">
                    Detailed view of {selectedRecommendation.title} at{" "}
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {personalPreferences.preferredIndustries.map(
                          (industry, index) => (
                            <span
                              key={index}
        ...
        <div className="mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <div className="flex items-center gap-2">
              ...
              {analyzeStatus && (
                <span className="text-sm text-gray-500">{analyzeStatus}</span>
              )}
            </div>
          </div>
        </div>
