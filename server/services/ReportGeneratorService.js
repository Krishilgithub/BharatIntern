...
class ReportGeneratorService {
  ...
  async generateCandidateReport(candidateData) {
    ...
    // Header
    await this.addReportHeader(doc, 'Candidate Profile Report', personalInfo.name);
    ...
    if (resumeAnalysis) {
      this.addSection(doc, 'Resume Analysis', () => {
        doc.fontSize(12)
           ...
           .text(`Key Skills: ${resumeAnalysis.skills?.slice(0, 10).join(', ') || 'None detected'}`)
           .moveDown();
    ...
    if (skillAssessments?.length > 0) {
      this.addSection(doc, 'Skill Assessment Results', () => {
        skillAssessments.forEach(assessment => {
          doc.fontSize(12)
             .text(`${assessment.title}: ${assessment.score}% (${assessment.level})`)
             .text(`Completed: ${new Date(assessment.completedAt).toLocaleDateString()}`)
      ...
      this.addSection(doc, 'Interview Performance', () => {
        interviewResults.forEach(interview => {
          doc.fontSize(12)
             ...
             .text(`Communication: ${interview.scores?.communication || 'N/A'}/100`)
             .text(`Technical Skills: ${interview.scores?.technical || 'N/A'}/100`)
             .text(`Problem Solving: ${interview.scores?.problemSolving || 'N/A'}/100`)
    ...
    if (codingProfile) {
      this.addSection(doc, 'Coding Profile Analysis', () => {
        doc.fontSize(12)
           .text(`GitHub Profile: ${codingProfile.github?.username || 'Not connected'}`)
           .text(`Public Repositories: ${codingProfile.github?.publicRepos || 0}`)
           .text(`LeetCode Problems Solved: ${codingProfile.leetcode?.totalSolved || 0}`)
           .text(`LeetCode Ranking: ${codingProfile.leetcode?.ranking || 'Unranked'}`)
           .text(`Primary Languages: ${codingProfile.primaryLanguages?.join(', ') || 'None detected'}`)
  ...
  async generateCompanyAnalyticsReport(companyData) {
    ...
    this.addSection(doc, 'Key Performance Indicators', () => {
      const conversionRate = ((hiringStats.hiresCompleted / hiringStats.totalApplications) * 100).toFixed(2);
      const interviewToOfferRate = ((hiringStats.offersExtended / hiringStats.totalInterviews) * 100).toFixed(2);
    ...
    if (candidateAnalytics) {
      this.addSection(doc, 'Candidate Analytics', () => {
        doc.fontSize(12)
           .text(`Top Skills in Demand: ${candidateAnalytics.topSkills?.join(', ') || 'None'}`)
           .text(`Average Experience Level: ${candidateAnalytics.avgExperience || 0} years`)
           .text(`Most Common Locations: ${candidateAnalytics.topLocations?.join(', ') || 'None'}`)
           .text(`Education Levels: ${candidateAnalytics.educationBreakdown || 'Not analyzed'}`)
           .moveDown();
      });
    }
    ...
    if (interviewMetrics) {
      this.addSection(doc, 'Interview Performance Metrics', () => {
        doc.fontSize(12)
           .text(`Average Interview Score: ${interviewMetrics.avgScore || 0}/100`)
           .text(`Technical Assessment Average: ${interviewMetrics.avgTechnicalScore || 0}/100`)
           .text(`Communication Score Average: ${interviewMetrics.avgCommunicationScore || 0}/100`)
           .text(`Average Interview Duration: ${interviewMetrics.avgDuration || 0} minutes`)
           .moveDown();
      });
    }
  ...
  async generateAssessmentReport(assessmentData) {
    ...
    this.addSection(doc, 'Score Distribution', () => {
      const scoreRanges = {
        ...
        'Good (80-89)': participants.filter(p => p.score >= 80 && p.score < 90).length,
        'Average (70-79)': participants.filter(p => p.score >= 70 && p.score < 80).length,
        'Below Average (60-69)': participants.filter(p => p.score >= 60 && p.score < 70).length,
    ...
    if (questionAnalytics?.length > 0) {
      this.addSection(doc, 'Question Performance Analysis', () => {
        questionAnalytics.forEach((question, index) => {
          doc.fontSize(11)
             .text(`Question ${index + 1}: ${question.text.substring(0, 100)}...`)
             .text(`Correct Answers: ${question.correctAnswers}/${participants.length} (${question.correctPercentage}%)`)
  ...
  async createCandidatesExcelSheet(workbook, candidates) {
    ...
    candidates.forEach(candidate => {
      worksheet.addRow({
        ...
        lastActive: candidate.lastActive ? new Date(candidate.lastActive).toLocaleDateString() : 'N/A',
