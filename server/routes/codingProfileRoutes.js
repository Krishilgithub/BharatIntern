...
const portfolioUpload = multer({
	...
	fileFilter: (req, file, cb) => {
		...
		if (allowedMimeTypes.includes(file.mimetype)) {
			...
			cb(
				new Error("Invalid file type. Allowed: PDF, JSON, TXT, ZIP, Images"),
				false
			);
		}
	},
 ...
 */
router.post("/github/analyze", verifyToken, async (req, res) => {
	try {
		const {
			username,
		...
		const analysis = await scraperService.analyzeGitHubProfile(
			username,
			analysisOptions
		);
 ...
 * @access Private (All authenticated users)
 */
router.post("/leetcode/analyze", verifyToken, async (req, res) => {
	try {
		const {
			username,
		...
		const analysis = await scraperService.analyzeLeetCodeProfile(
			username,
			analysisOptions
		);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const analysis = await scraperService.analyzePortfolio(
				req.files,
				analysisOptions
			);
 ...
 * @access Private (All authenticated users)
 */
router.post("/multi-platform", verifyToken, async (req, res) => {
	try {
		const {
			platforms = {},
		...
		const providedPlatforms = Object.keys(platforms).filter((p) =>
		...
		const analysis = await scraperService.analyzeMultiplePlatforms(
			platforms,
			analysisOptions
		);
 ...
 * @access Private (All authenticated users)
 */
router.post("/skills/validate", verifyToken, async (req, res) => {
	try {
		const {
			claimedSkills = [],
			platforms = {},
 ...
 */
router.get(
	"/repository/:platform/:username/:repo",
	verifyToken,
	async (req, res) => {
		try {
 ...
 * @access Private (All authenticated users)
 */
router.post("/competitive/analyze", verifyToken, async (req, res) => {
	try {
		const {
			platforms = {},
			includeRatings = true,
		...
		const providedPlatforms = Object.keys(platforms).filter((p) =>
			competitivePlatforms.includes(p)
		);
		...
		const analysis = await scraperService.analyzeCompetitiveProgramming(
			platforms,
			analysisOptions
		);
		...
		res.status(500).json({
			success: false,
			message:
				error.message || "Failed to analyze competitive programming profiles",
		});
	}
});
 ...
 */
router.get("/profiles/:candidateId", verifyToken, async (req, res) => {
	try {
		...
		const profiles = await scraperService.getCandidateProfiles(
			candidateId,
			options
		);
...
router.post(
	...
	async (req, res) => {
		try {
			...
			const analysis = await scraperService.bulkAnalyzeProfiles(
				candidates,
				analysisOptions
			);
 ...
 * @access Private (All authenticated users)
 */
router.post("/skills/extract", verifyToken, async (req, res) => {
	try {
		const {
			profileData = {},
			repositories = [],
		...
		if (Object.keys(profileData).length === 0 && repositories.length === 0) {
			return res.status(400).json({
				success: false,
				message:
					"Profile data or repositories are required for skill extraction",
			});
		}
...
router.post("/refresh", verifyToken, async (req, res) => {
	try {
		...
		const refreshResult = await scraperService.refreshProfiles(
			platforms,
			refreshOptions
		);
 ...
 * @access Private (Owner/Company/Admin)
 */
router.get("/analytics/:candidateId", verifyToken, async (req, res) => {
	try {
		const { candidateId } = req.params;
		const {
			period = "1y",
		...
		const analytics = await scraperService.getCodingAnalytics(
			candidateId,
			analyticsOptions
		);
